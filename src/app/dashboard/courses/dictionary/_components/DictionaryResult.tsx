...
export default function DictionaryResult({ entry, playAudio }: DictionaryResultProps) {
  ...
  // Kiểm tra tính hợp lệ của cấu trúc dữ liệu AI bọc bên trong entry
  ...
  return (
    <div className="w-full max-w-[720px] mx-auto space-y-4 font-['DM_Sans'] antialiased selection:bg-[#FF339920]">
      {/* Thanh Tiêu Đề Từ Vựng Cốt Lõi */}
      <div className="bg-[#2d3436] text-[#f7f9f8] px-6 py-4 flex items-center justify-between rounded-full">
        <div className="flex items-center gap-2 text-[#00cec9] font-['Nunito'] font-[900] text-sm uppercase tracking-wider pl-1">
          {entry.word}
        </div>
      </div>

      {/* Danh Sách Các Từ Loại trích xuất trực tiếp theo cấu trúc phẳng */}
      <div className="space-y-4">
        {entry.ai.partsOfSpeech.map((posGroup, posIdx) => {
          ...
          return (
            <div key={posIdx} className="bg-white p-6 sm:p-8 rounded-[24px] space-y-5 shadow-xs">
              {/* Tag định dạng Từ loại */}
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#2d3436] text-[#f7f9f8] font-['Nunito'] font-[800] text-xs uppercase tracking-widest">
                <Layers size={14} className="text-[#00cec9]" strokeWidth={2.5} />
                {posGroup.partOfSpeech}
              </div>

              {/* Nội dung ngữ nghĩa phẳng trích xuất trực tiếp */}
              <div className="space-y-4 bg-[#f7f9f8] p-5 rounded-[20px]">
                <div className="flex gap-4 items-start px-1">
                  <div className="w-8 h-8 rounded-full bg-[#00cec920] flex items-center justify-center flex-shrink-0 text-[#00cec9]">
                    <AlignLeft size={16} strokeWidth={2.5} />
                  </div>
                  <div className="space-y-1.5 pt-0.5">
                    <p className="text-base text-[#2d3436] font-[800] leading-snug">
                      {posGroup.definitionEn}
                    </p>
                    <p className="text-sm text-[#FF3399] font-[800] tracking-wide">
                      {posGroup.definitionVi}
                    </p>
                  </div>
                </div>

                {/* Hiển thị ví dụ minh họa kèm nút phát âm từng câu */}
                {posGroup.examples && posGroup.examples.length > 0 && (
                  <div className="space-y-2.5 pt-2 border-t border-[#e9eceb]">
                    {posGroup.examples.map((example, exIdx) => (
                      <div key={exIdx} className="bg-white p-4 rounded-[16px] flex gap-3 items-start shadow-xs group relative justify-between">
                        <div className="flex gap-3 items-start w-full pr-8">
                          <div className="w-7 h-7 rounded-full bg-[#FF339915] flex items-center justify-center flex-shrink-0 text-[#FF3399]">
                            <Quote size={12} strokeWidth={2.5} className="fill-[#FF3399]" />
                          </div>
                          <div className="space-y-1 w-full">
                            <p className="text-sm text-[#2d3436] font-[500] italic leading-relaxed">
                              "{example.en}"
                            </p>
                            <p className="text-xs text-zinc-500 font-[500] flex items-center gap-1">
                        ...
                        {/* Nút loa phát âm giọng Mỹ cho từng câu ví dụ tương ứng */}
                        <button
                          type="button"
                          onClick={() => {
